...
function renderMG() {
  ...
  el.innerHTML =
    ...
    + '<div class="gen-badge">' + remaining + ' left</div>'
    + '</div>'
    + '<div class="mg-body">'
...
function renderMGPhotoInput() {
  return '<div class="mg-input-box" id="mg-photo-box">'
    ...
    + '<div id="mg-photo-preview" style="display:none"><img id="mg-photo-img" style="max-width:100%;border-radius:12px;max-height:200px;object-fit:contain"/><button class="mg-clear-btn" onclick="clearPhoto()">✕ Clear</button></div>'
...
function mgTab(t) {
  ...
  if (t === 'photo') {
    ...
    wrap.innerHTML = '<textarea id="mg-text-inp" class="mg-textarea" placeholder="Paste or type your study content here...\n\nE.g. notes, paragraphs from books, topics you want questions on..."></textarea>';
  }
}

function setMgCount(n) {
  mgCount = n;
    ...
    if (!r.ok) {
      if (d.error === 'trial_expired' || d.error === 'limit_reached') {
        if (resultsEl) resultsEl.innerHTML = '<div class="mg-locked" style="margin-top:0">'
          ...
          + '<div style="font-size:13px;color:var(--t2);margin-bottom:16px">' + d.message + '</div>'
          + '<button class="btn btn-trial" onclick="window.open(\'https://rzp.io/rzp/zJ6jF8B\',\'_blank\')">Upgrade to Pro →</button>'
          + '</div>';
      } else {
        toast(d.error || 'Generation failed. Try again.', 'err');
